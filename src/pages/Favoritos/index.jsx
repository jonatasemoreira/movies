...
export default function Favoritos () {
    ...
    return (
        <div id="meus-filmes" className='meus-filmes'>
          <h1>Filmes Salvos</h1>

            { /* Se a quantidade de itens no nosso array for igual a zero, quer dizer que não temos filmes na lista */ }
            {filmes.length === 0 && <span><p>Você não possui filme salvo :( </p></span>}

            <ul className='lista-de-salvos'>
              {filmes.map((item) => {
                return (
                  <li key={item.id}>
                    <span>{item.nome}</span>
                      <div className='detalhes-salvos'>
                        <Link className='detalheFilm' to={`/filme/${item.id}`}>Detalhes</Link>
